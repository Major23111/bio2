@if ($order)
    ...
    <div class="flex items-center justify-between gap-3 pt-5 border-t border-slate-200">
        ...
        <a href="{{ route('orders.invoice.download', ['orderId' => encrypt_url_value($order->id)]) }}" target="_blank" class="inline-flex items-center gap-2 text-[13px] font-semibold text-slate-500 hover:text-slate-800 transition cursor-pointer px-3 py-2 rounded-xl hover:bg-slate-100">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
