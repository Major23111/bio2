<aside id="admin-sidebar" class="w-64 flex-shrink-0">
    <nav
        class="sticky top-24 space-y-1.5 bg-[var(--ui-surface)] p-4 rounded-2xl shadow-[var(--ui-shadow-soft)] border border-[var(--ui-border)] pb-6">

        <h3 class="px-3 text-xs font-bold uppercase tracking-widest text-slate-400 mb-4 mt-2">Admin Portal</h3>

        <a href="{{ route('admin.dashboard') }}"
            class="flex items-center gap-3 px-3 py-3 rounded-xl {{ request()->routeIs('admin.dashboard') ? 'bg-primary-600 text-white' : 'text-[var(--ui-text-muted)] hover:bg-[var(--ui-surface-subtle)] hover:text-[var(--ui-text)]' }} font-bold text-[13px] transition w-full relative">
            ...
            @else
                <svg class="h-5 w-5 z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
        ...
        @php
            ...
            $navLinks = [
                ...
                ['icon' => 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10', 'title' => 'Inventory', 'route' => 'admin.inventory.index'],
                ...
                //['icon' => 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z', 'title' => 'System Settings', 'route' => 'admin.ui-fields-modification'],
        ...
        @foreach($navLinks as $link)
            ...
            <a href="{{ isset($link['route']) ? route($link['route']) : '#' }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-semibold text-[13px] transition w-full relative {{ $isActive ? 'bg-primary-600 text-white font-bold' : 'text-[var(--ui-text-muted)] hover:bg-[var(--ui-surface-subtle)] hover:text-[var(--ui-text)]' }}">
                @if($isActive)
                    <div class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-white rounded-r-md"></div>
                    <svg class="h-5 w-5 text-indigo-200 z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="{{ $link['icon'] }}"></path>
                    </svg>
                @else
                    <svg class="h-5 w-5 text-slate-400 z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="{{ $link['icon'] }}"></path>
                    </svg>
                @endif
                ...
                @if(isset($link['badge']))
                    ...
                    <span
                        class="text-[9px] font-black rounded-md px-1.5 py-0.5 z-10 {{ $isActive ? 'bg-white/20 text-white' : ($bc === 'emerald' ? 'bg-primary-50 text-primary-600' : 'bg-primary-50 text-primary-600') }}">{{ $link['badge'] }}</span>
                @endif
            </a>
        @endforeach
        ...
        @php
            ...
            if (empty($sidebarUserInitials))
                $sidebarUserInitials = 'A';
        @endphp
        <div class="mt-6 pt-4 border-t border-[var(--ui-border)]">
            <div class="flex items-center gap-3 px-3 py-2">
                <div
                    class="h-9 w-9 rounded-full bg-primary-600 text-white flex items-center justify-center text-[11px] font-black flex-shrink-0">
                    {{ $sidebarUserInitials }}</div>
                ...
                <form method="POST" action="{{ route('logout') }}" class="flex-shrink-0">
                    @csrf
                    <button type="submit"
                        class="h-8 w-8 rounded-lg hover:bg-rose-50 text-slate-400 hover:text-rose-500 transition flex items-center justify-center cursor-pointer"
                        title="Logout">
