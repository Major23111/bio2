...
@php
    ...
    $periodFilters = ['Last 3 months', 'Last 6 months'];
    $statusStyles = [
        'delivered' => [
...
<x-account.workspace :portal="$portal" active="orders" title="Order History"
    ...
    <section class="{{ $compactPanelClass }}">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-end">
            ...
            <div class="min-w-0 lg:ml-auto lg:w-full lg:max-w-[35%]">
                <label for="order-range"
                    class="block text-[11px] font-bold uppercase tracking-[0.22em] text-slate-400">Date Range</label>
    ...
    <div class="space-y-5">
        ...
        <article class="{{ $panelClass }}">
            <div class="grid gap-6 xl:grid-cols-[auto_minmax(0,1fr)_auto] xl:items-center">
                <div
                    class="flex h-24 w-24 items-center justify-center overflow-hidden rounded-2xl {{ $order['image_background'] }} p-3 shadow-inner">
                    <img src="{{ $order['image'] }}" alt="{{ $order['product_name'] ?? 'Order' }}"
                        class="h-full w-full rounded-xl object-cover mix-blend-multiply">
                </div>

                <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-3">
                        <span
                            class="inline-flex rounded-lg px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wider {{ $statusStyle['badge'] }}">
                            {{ ucfirst($order['status']) }}
                        </span>
                        <span class="text-[12px] font-bold tracking-tight text-slate-400">#
                            ORD-{{ str_pad((string) $order['id'], 6, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-slate-500">
                        ...
                        <span class="text-lg font-bold text-slate-900">{{ $order['currency'] }}
                            {{ number_format($order['total_amount'], 2) }}</span>
                        <span
                            class="inline-flex items-center rounded-full bg-slate-50 px-3 py-1 text-[12px] font-semibold text-slate-500">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" onclick="openOrderModal({{ $index }})"
